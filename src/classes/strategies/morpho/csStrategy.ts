...
/**
 * @notice CSStrategy - ClearStar USDC Reactor vault
 * @notice MetaMorpho vault curated by ClearStar for optimized USDC yield
 */
export class CSStrategy extends BaseStrategy<typeof MORPHO> {
  private readonly VAULT_ADDRESS = "0x1D3b1Cd0a0f242d598834b3F2d126dC6bd774657";

  constructor(chainId: GetProtocolChains<typeof MORPHO>) {
    super(chainId, MORPHO, "CSStrategy");
  }
  ...
  async investCalls(
    ...
    if (!asset)
      throw new Error("CSStrategy: asset parameter is required");
  ...
  async redeemCalls(
    ...
    try {
      console.log(`🔄 CSStrategy Withdrawal Request:`, {
      ...
      console.error("❌ CSStrategy redeemCalls error:", error);
      ...
      try {
        ...
        throw new Error(`CSStrategy withdrawal failed: ${fallbackError}`);
      }
    }
  }
  ...
  async getProfit(user: Address, position: Position): Promise<number> {
    try {
      ...
      console.error("Error calculating CS profit:", error);
  ...
  getVaultInfo() {
    return {
      ...
      riskLevel: "Medium",
      description: "USDC reactor vault curated by ClearStar for optimized yield",
      externalLink: `https://app.morpho.org/base/vault/${this.VAULT_ADDRESS}/clearstar-usdc-reactor`,
    };
  }
}
