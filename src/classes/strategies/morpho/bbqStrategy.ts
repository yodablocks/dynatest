...
/**
 * @notice BBQStrategy - Steakhouse High Yield USDC v11 vault
 * @notice Institutional-grade MetaMorpho vault curated by Steakhouse Financial
 */
export class BBQStrategy extends BaseStrategy<typeof MORPHO> {
  private readonly VAULT_ADDRESS = "0xBEEFA7B88064FeEF0cEe02AAeBBd95D30df3878F";

  constructor(chainId: GetProtocolChains<typeof MORPHO>) {
    super(chainId, MORPHO, "BBQStrategy");
  }
  ...
  async investCalls(
    ...
    if (!asset)
      throw new Error("BBQStrategy: asset parameter is required");
  ...
  async redeemCalls(
    ...
    try {
      console.log(`🔄 BBQStrategy Withdrawal Request:`, {
      ...
      console.error("❌ BBQStrategy redeemCalls error:", error);
      ...
      try {
        ...
        throw new Error(`BBQStrategy withdrawal failed: ${fallbackError}`);
      }
    }
  }
  ...
  async getProfit(user: Address, position: Position): Promise<number> {
    try {
      ...
      console.error("Error calculating BBQ profit:", error);
  ...
  getVaultInfo() {
    return {
      ...
      riskLevel: "Medium",
      description: "High-yield USDC vault curated by Steakhouse Financial",
      externalLink: `https://app.morpho.org/base/vault/${this.VAULT_ADDRESS}/steakhouse-high-yield-usdc-v11`,
    };
  }
}
