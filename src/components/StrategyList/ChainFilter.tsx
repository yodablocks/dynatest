...
export default function ChainFilter({
  ...
  const getChainStatus = (chainId: number) => {
    // Base, Celo, BSC, Polygon, and Arbitrum are live
    return (chainId === base.id || chainId === celo.id || chainId === bsc.id || chainId === polygon.id || chainId === arbitrum.id) ? 'active' : 'coming_soon';
  };
